...
const HomePage: NextPage = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                ...
                const userLang = window.location.search?.split("?lang=")[1] ?? "en"
                setLang(userLang)
                setPosts(data);
    ...
    return (
        <main className="mx-auto px-6 py-10 flex flex-col lg:flex-row gap-10">
            {/* Sidebar */}
            <aside
                className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 top-10 self-start shadow-sm">
                <h2 className="text-xl font-semibold mb-5 border-b border-zinc-300 dark:border-zinc-700 pb-2">
                    Guide Categories
                </h2>
                <div className="space-y-3 w-72 h-auto">
                                ...
                                {isExpanded && (
                                    ...
                                    >
                                        {postsInType.map((post) => (
                                            <li key={post.slug}>
                                                <button
                                                    onClick={() => window.location.href = `/guide/${post.slug}${lang ? `?lang=${lang == "de" ? "de" : "en"}` : ""}`}
